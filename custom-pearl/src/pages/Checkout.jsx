...
const Checkout = () => {
  ...
  const [formData, setFormData] = useState({
    customerName:'', customerPhone:'', shippingAddress:'', orderChannel:'Website', paymentMethod:'',
  });
  ...
  const validateStep2 = () => {
    ...
    if (isOnline && !transactionId.trim()) { setBanner({ type:'error', msg:'Please enter your Transaction / Reference ID after making the payment.' }); return false; }
    setBanner({ type:'', msg:'' }); return true;
  };
      ...
      setTimeout(() => {
        if (channel === 'WhatsApp') {
          const itemsList = cartItems.map(i =>
            `• ${i.Name} × ${i.qty}${i.Price > 0 ? ` — Rs. ${Number(i.Price)*i.qty}` : ''}`
          ).join('%0A');
          const msg =
            `Hi Custom Pearl! Confirming my order.%0A%0A` +
            `Name: ${formData.customerName}%0A` +
            `Phone: ${formData.customerPhone}%0A` +
            `Address: ${formData.shippingAddress}%0A%0A` +
            `Items:%0A${itemsList}%0A%0A` +
            `Total: Rs. ${getCartTotal()}%0A` +
            `Payment: ${PAYMENT_LABELS[formData.paymentMethod]||formData.paymentMethod}%0A` +
            (transactionId ? `TXN ID: ${transactionId}%0A` : '') +
  ...
  if (!cartItems || cartItems.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 min-h-screen bg-white dark:bg-gray-900 transition-colors">
        ...
        <Link to="/shop" className="bg-pink-600 text-white px-6 py-3 rounded-full hover:bg-pink-700 font-semibold transition">
          Browse Collection
        </Link>
      </div>
    );
      ...
      <div className="flex flex-col items-center justify-center py-20 px-4 min-h-screen bg-white dark:bg-gray-900 transition-colors">
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-2xl p-10 max-w-md w-full text-center">
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 mb-6 text-sm text-left">
            ...
            {orderDone.paymentMethod === 'cod' && (
              <p className="text-xs text-gray-400 mt-1">Payment due on delivery.</p>
            )}
          </div>

          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button onClick={() => navigate('/track')}
              className="bg-pink-600 hover:bg-pink-700 text-white py-3 px-6 rounded-xl font-bold transition text-sm">
              📦 Track This Order
            </button>
            <button onClick={() => navigate('/')}
              className="border border-pink-600 text-pink-600 dark:text-pink-400 py-3 px-6 rounded-xl font-bold hover:bg-pink-50 dark:hover:bg-pink-900/20 transition text-sm">
              Back to Home
            </button>
          </div>
        </div>
          ...
          <React.Fragment key={s}>
            <div className={`flex items-center gap-2 ${i < step-1 ? 'text-green-600' : i===step-1 ? 'text-pink-600' : 'text-gray-300 dark:text-gray-600'}`}>
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 ${
                i < step-1  ? 'border-green-500 bg-green-500 text-white'
                : i===step-1 ? 'border-pink-600 bg-pink-600 text-white'
            ...
            {i < STEPS.length-1 && (
              <div className={`flex-1 h-0.5 ${i < step-1 ? 'bg-green-400' : 'bg-gray-200 dark:bg-gray-700'}`} />
            )}
          </React.Fragment>
        ))}
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 space-y-5">
          ...
          <div className={`bg-white dark:bg-gray-800 border rounded-xl p-6 shadow-sm ${step===1 ? 'border-pink-400 dark:border-pink-500' : 'border-gray-200 dark:border-gray-700'}`}>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-gray-800 dark:text-white flex items-center gap-2">
                <span className={`w-6 h-6 rounded-full text-xs font-bold flex items-center justify-center ${step>1?'bg-green-500 text-white':'bg-pink-600 text-white'}`}>
                  {step>1?'✓':'1'}
                </span>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea name="shippingAddress" value={formData.shippingAddress} onChange={handleChange}
                    placeholder="House number, street, area, city, province" rows={3}
          ...
          {step >= 2 && (
            <div className={`bg-white dark:bg-gray-800 border rounded-xl p-6 shadow-sm ${step===2?'border-pink-400 dark:border-pink-500':'border-gray-200 dark:border-gray-700'}`}>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-bold text-gray-800 dark:text-white flex items-center gap-2">
                  <span className={`w-6 h-6 rounded-full text-xs font-bold flex items-center justify-center ${step>2?'bg-green-500 text-white':'bg-pink-600 text-white'}`}>
                    {step>2?'✓':'2'}
                  </span>
                  Payment Method
                </h3>
              ...
              {step===2 ? (
                <div className="space-y-3">
                  {paymentMethods.map(pm => (
                    <label key={pm.id}
                      className={`flex items-start gap-4 p-4 rounded-xl border-2 cursor-pointer transition ${
                        formData.paymentMethod===pm.id ? 'border-pink-500 bg-pink-50 dark:bg-pink-900/20' : 'border-gray-200 dark:border-gray-600 hover:border-pink-300'
                      }`}>
                      <input type="radio" name="paymentMethod" value={pm.id}
                        checked={formData.paymentMethod===pm.id} onChange={handleChange} className="accent-pink-600 mt-0.5" />
                      <div className="flex-1">
                        ...
                        {formData.paymentMethod===pm.id && pm.detail && (
                          <div className="mt-3 bg-white dark:bg-gray-700 rounded-xl p-3 border border-gray-200 dark:border-gray-600 text-xs space-y-1.5">
                            ...
                            {pm.detail.bankName && (
                              <p className="text-gray-600 dark:text-gray-300">Bank: <strong>{pm.detail.bankName}</strong></p>
                            )}
                            <p className="text-gray-600 dark:text-gray-300">
                              Account Title: <strong>{pm.detail.accountTitle || 'Custom Pearl'}</strong>
                            </p>
                            <p className="text-gray-600 dark:text-gray-300">
                              {pm.id==='bank' ? 'IBAN' : 'Number'}:{' '}
                              <strong className="font-mono tracking-wide">{pm.detail.accountNumber || '—'}</strong>
                            </p>
                            <p className="text-pink-600 dark:text-pink-400 font-bold mt-1">
                              Amount to send: Rs. {getCartTotal().toLocaleString()}
                            </p>
                            <div className="pt-1">
                              <label className="block text-gray-700 dark:text-gray-200 font-semibold mb-1">
                                Transaction / Reference ID <span className="text-pink-500">*</span>
                              </label>
                              <input type="text" value={transactionId} onChange={e => setTransactionId(e.target.value)}
                                placeholder="Enter TXN ID after making payment"
                                className="w-full border border-gray-300 dark:border-gray-500 rounded-lg p-2 text-xs bg-white dark:bg-gray-600 dark:text-white focus:outline-none focus:ring-2 focus:ring-pink-400" />
                            </div>
                          </div>
                        )}
                      </div>
                  ...
                  <button onClick={() => { if (validateStep2()) setStep(3); }}
                    className="w-full bg-pink-600 hover:bg-pink-700 text-white py-3 rounded-xl font-bold text-sm transition mt-2">
                    Continue to Confirm →
                  </button>
                </div>
              ) : (
          ...
          {step >= 3 && (
            <div className="bg-white dark:bg-gray-800 border border-pink-400 dark:border-pink-500 rounded-xl p-6 shadow-sm">
              <h3 className="text-lg font-bold text-gray-800 dark:text-white flex items-center gap-2 mb-1">
                <span className="w-6 h-6 rounded-full text-xs font-bold flex items-center justify-center bg-pink-600 text-white">3</span>
                Confirm Your Order
              </h3>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <button key={ch.channel} onClick={() => placeOrder(ch.channel)} disabled={loading}
                    ...
                    {loading && <svg className="animate-spin h-4 w-4 ml-auto" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>}
                  </button>
                ))}
              </div>
            </div>
        ...
        <div className="w-full lg:w-80">
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 shadow-sm sticky top-24">
            <h3 className="text-lg font-bold text-gray-800 dark:text-white mb-4 border-b border-gray-200 dark:border-gray-700 pb-3">
              Order Summary
            </h3>
                ...
                return (
                  <div key={item.Id} className="flex items-center gap-3 text-sm">
                    <div className="w-10 h-10 rounded-lg bg-pink-50 dark:bg-gray-700 flex-shrink-0 flex items-center justify-center overflow-hidden">
                      {imgSrc ? <img src={imgSrc} alt={item.Name} className="w-full h-full object-cover" /> : <span>{item.isCustom?'✨':'👜'}</span>}
                    </div>
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-3 space-y-1.5">
              <div className="flex justify-between text-sm text-gray-500 dark:text-gray-400">
                <span>Subtotal</span><span>Rs. {getCartTotal().toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-sm text-gray-500 dark:text-gray-400">
              ...
              {formData.paymentMethod && (
                <div className="flex justify-between text-sm text-gray-500 dark:text-gray-400">
                  <span>Payment</span>
                  <span>{paymentMethods.find(p=>p.id===formData.paymentMethod)?.label}</span>
                </div>
              )}
            ...
            <Link to="/cart" className="block text-center text-xs text-gray-400 hover:text-gray-600 mt-4 underline">
              ← Edit Cart
            </Link>
          </div>
        </div>
